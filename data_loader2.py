...
class EEGDataLoader(Dataset):
    ...
    def split_dataset(self):
        ...
        # print(data_fname_dict)
        # print(file_idx)

    
        for i in range(len(data_fname_list)):

            # Gets the subject index from the file name 
            subject_idx = int(data_fname_list[i][3:5])
            # print(subject_idx)
            if subject_idx in split_idx_list[fold - 1][self.set]:
        ...
        # print(inputs[10][10:10+seq_len])
        # print(labels[10][10:10+seq_len])
        return inputs, labels, epochs

train_dataset = EEGDataLoader(set='train')
